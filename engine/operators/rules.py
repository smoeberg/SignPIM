"""
Native port of legacy/rule_evaluator.py — same semantics, integrated with
the engine's operator registry. Used by the quality scoring pipeline.
...
logger = logging.getLogger("signpim.rules")
...
def _as_image_list(value: Any) -> list:
    """Normalize a possibly-stringified images field into a list."""
...
def _rule_missing_images(product: Dict[str, Any], parameters: Dict[str, Any]) -> bool:
    ...
    if isinstance(images, str):
        stripped = images.strip()
        if not stripped:
            return True  # empty string = no images
        try:
            images = json.loads(stripped)
        except (json.JSONDecodeError, TypeError):
            images = [stripped]
...
def _rule_invalid_image_url(product: Dict[str, Any], parameters: Dict[str, Any]) -> bool:
    images = _as_image_list(product.get('images', []))
    for img in images:
